...
//Solapas del legajo 
$xcrud->fields('db1,db2,db3,db9,db10,db11,db12,db13,db14,db15,db16,db23a1,db7,C24,C25,C25other,C251,C26,C27,C29,C291,c30,c30other,c32,c33,c34,C36,C37,PA1,PA2,PA3,PA4,DI2,DI3,DI4,DI5,DI7,P1,P1other,CO2,co3filecount,CO1,PROVINCIA,DEPARTAMENTO,LOCALIDAD', false, 'Perfil del postulante');
...
//$xcrud->where('co3filecount =', 0); // 0 = No cargo carta AVAL. (para marcar clausula OR , se utiliza or_where , si fuera AND es solo where.)
$xcrud->where("c35 = 'Participé' OR co3filecount = 0"); // Participé = Ya participo de GCP / 0 = No subio carta aval

//$xcrud->where('c35 =', "Participé");
//$xcrud->or_where('co3filecount =', 0);
//$xcrud->where('co3filecount =', 0);

//$xcrud->subselect('Order total','SELECT SUM(priceEach) FROM orderdetails WHERE orderNumber = {orderNumber}'); // insert as last column

//$xcrud->subselect('Para evaluar','SELECT SUM(priceEach) FROM orderdetails WHERE orderNumber = {orderNumber}'); // insert as last column
//$xcrud->subselect('Para_evaluar','SELECT COUNT( c35 ) FROM gcp2021 g WHERE g.c35 = {Participé} '); // insert as last column

//$xcrud->subselect( 'Para_evaluar', 'SELECT * FROM `gcp2021` WHERE `gcp2021`.`c35` LIKE "Participé" ');
//$xcrud->where('c35 =', "Participé");
//$xcrud->or_where('co3filecount =', 0);
///SELECT * FROM `gcp2021` WHERE `gcp2021`.`C35`='Participé' 
