...
d3.csv("base_para_dimesiones_gcp_centro.csv").then(function(experiments) {
   ...
   var  ndx = crossfilter(experiments),// 1 Crea la instancia de filtro cruzado.
    ...
    .on('pretransition', function(chart) {
        chart.selectAll('text.pie-slice').text(function(d) {
            if (((d.endAngle - d.startAngle) / (2*Math.PI) * 100) >= 2.40){ //Si el porcentaje obtenido es mayor a 0.1 se muestra
                return d.data.key + ' = ' +  dc.utils.printSingleValue((d.endAngle - d.startAngle) / (2*Math.PI) * 100) + '%';
            }})
    });
