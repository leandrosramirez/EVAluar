var graficotorta   = new dc.PieChart("#graficoxregiones");
  ...
  d3.csv("archivos/base_para_dimesiones.csv").then(function(experiments) {
...
edad
    ...
    .on('pretransition', function(chart) {
        chart.selectAll('text.pie-slice').text(function(d) {
            if (((d.endAngle - d.startAngle) / (2*Math.PI) * 100) >= 2.50){ //Si el porcentaje obtenido es mayor a 0.1 se muestra
...
torta
    ...
    .on('pretransition', function(chart) {
        chart.selectAll('text.pie-slice').text(function(d) {
            if (((d.endAngle - d.startAngle) / (2*Math.PI) * 100) >= 1.60){ //Si el porcentaje obtenido es mayor a 0.1 se muestra
                return d.data.key + ' = ' +  dc.utils.printSingleValue((d.endAngle - d.startAngle) / (2*Math.PI) * 100) + '%';
            }})
    });
